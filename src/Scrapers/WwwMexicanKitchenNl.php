 ...
 */
class WwwMexicanKitchenNl extends SchemaOrgJsonLd
{
    /**
    ...
    public function supports(Crawler $crawler) : bool
    {
        ...
        ) && 'www.mexicankitchen.nl' === parse_url($crawler->getUri(), PHP_URL_HOST);
    }
    ...
    protected function extractImage(Crawler $crawler, array $json)
    {
        ...
        $image = array_filter( $image , function( $item ) { return $item !== ''; } );

        if ( sizeof( $image ) > 0 ) return $image[0];
    ...
    protected function extractInstructions(Crawler $crawler, array $json)
    {
        ...
        $instructions = array_filter($instructions, function ($instruction) {
            return array_key_exists('text', $instruction);
        });
